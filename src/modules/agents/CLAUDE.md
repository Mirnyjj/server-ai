...
Каждый вызов разрешённого AgentTool создаёт AgentAction со статусом RUNNING, затем фиксируется как SUCCESS с output либо FAILED с ошибкой. Попытка вызвать инструмент вне allowlist фиксируется как CANCELLED. В audit input сохраняются профиль, исходный запрос и фактические аргументы tool. BigInt в результатах сериализуется в строку.
