...
For Reel generation, intermediate scene frames and scene videos are tracked during the run and removed from Object Storage and MediaAsset after the final composed video is successfully attached to the Post. Cleanup is best-effort and cannot turn a successful composition into a failed pipeline.

External generated video downloads in the FFmpeg composer and Object Storage re-hosting use bounded HTTP requests to prevent a stuck provider URL from hanging a generation job indefinitely.
